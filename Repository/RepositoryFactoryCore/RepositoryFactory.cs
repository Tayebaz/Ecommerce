    ...
    public abstract class RepositoryFactory<TEntity> where TEntity : class
    {
        ...
        public List<TEntity> GetListWithNoTracking(Expression<Func<TEntity, bool>> exp = null, Func<IQueryable<TEntity>, IOrderedQueryable<TEntity>> orderby = null)
        {
            try
            {
                    ...
                    return query.AsNoTracking().ToList<TEntity>(); //non static method requires index
            }
            catch(Exception ex)
            {
                return null;
        ...
        public List<TEntity> GetList(Expression<Func<TEntity, bool>> exp = null, Func<IQueryable<TEntity>, IOrderedQueryable<TEntity>> orderby = null)
        {
            IQueryable<TEntity> query = _dbSet;
